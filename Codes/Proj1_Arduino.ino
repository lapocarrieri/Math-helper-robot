...
if(buf[1] > 16 && buf[1]<23){
  ...
  if(done == false){
      ...
      if(buf[1]==18){
        Serial.println("side square");
      } 
      if(buf[1]==19){
        Serial.println("Length times Breadth divided by two");
      } 
      if(buf[1]==20){
        Serial.println("Major Diagonal times minor diagonal by two");
      }
      if(buf[1]==21){
        Serial.println("Length times Breadth");
      }
      if(buf[1]==22){
        Serial.println("pi times radius to the square");
      }
  myVR.clear();
